...
const DetalleAlarma = () => {
  ...
  const obtenerDatos = async(e) => {
    ...
    try{
      ...
      let url2 = "https://www.protrack.ad105.net/api/reports/route?";
      let groupId;
      ...
      url2 = url2+"deviceId="+deviceId+"&";
      url2 = url2+"groupId="+groupId+"&type=allEvents&from="+dateStart+"&to="+dateEnd;
      console.log(url)
      const resultado2 = await fetch(`${url}`, requestOptions)
      const deviceData2 = await resultado2.json();
      const resultado3 = await fetch(`${url2}`, requestOptions)
      const deviceData3 = await resultado3.json();
      for(let i = 0; i < deviceData2.length; i++){
        for(let j = 0; j < deviceData3.length; j++){
          if(deviceData2[i].positionId === deviceData3[j].id){
            const url = `https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/reverseGeocode?f=pjson&featureTypes=&location=${deviceData3[j].longitude},${deviceData3[j].latitude}`
            const resultado = await axios.get(url);
            //return ubicacion.ShortLabel;
            console.log(resultado.data.address.LongLabel)
            ubicacion.push({id: i, type: "alarma", tipo: deviceData2[i].type, serverTime: deviceData2[i].serverTime, address: resultado.data.address.LongLabel, speed: deviceData3[j].speed});
          }
        }
     }
    ...
    <div className="h-screen bg-gray-200 flex flex-col">
      <ToastContainer 
        ...
        <nav className='w-full p-2 text-center text-white capitalize navColor flex items-center justify-between'>
            ...
            <h1 className='self-center'>Detalles de Alarma</h1>
            <span className=''></span>
        </nav>
        {
          !active ? (
