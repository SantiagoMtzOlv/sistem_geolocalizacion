...
const DetalleVel = () => {
  ...
  const obtenerDatos = async(e) => {
    ...
    try {
      ...
      const resulDev = await fetch(`https://www.protrack.ad105.net/api/devices/${deviceId}`, requestOptions)
      const resDev = await resulDev.json()
      speedLimit= resDev.attributes.speedLimit;
      

      console.log(deviceData2)
      for(let i = 0; i < deviceData2.length; i++){
          console.log(speedLimit)
          if(deviceData2[i].maxSpeed >= speedLimit){
            //obtener direccion
            ...
            ubicacion.push({id: i, lon: deviceData2[i].endLon, lat: deviceData2[i].endLat, address: resultado.data.address.LongLabel, name: deviceData2[i].deviceName, time: deviceData2[i].endTime, speed: deviceData2[i].maxSpeed, type: "velocidad"});
          }
       }
    ...
    <div className="h-screen bg-gray-200 flex flex-col">
      <ToastContainer 
        ...
        <nav className='w-full p-2 text-center text-white capitalize navColor flex items-center justify-between'>
            ...
            <h1 className='self-center'>Detalles de Exceso de Velocidad</h1>
            <span className=''></span>
        </nav>
        {
          !active ? (
